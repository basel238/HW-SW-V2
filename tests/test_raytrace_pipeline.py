...
class RaytracePipelineTests(unittest.TestCase):
    ...
    def test_time_only_compares_reference_to_selected_kernel(self):
        for kernel in (None, "guards", "combined", "upstream"):
            with self.subTest(kernel=kernel), tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertIn("UPSTREAM pyperformance raytrace kernel (unmodified)", base_raw)
                ...
                summary = results / "latest_comparison_raytrace/summary.txt"
                self.assertIn("SPEEDUP", summary.read_text())

    def test_invalid_or_unsupported_kernel_is_rejected_before_measurement(self):
        cases = [("script_raytrace.sh", "1", ["--kernel"]),
                 ("script_raytrace.sh", "1", ["--kernel", "missing"]),
                 ("script_raytrace.sh", "0", ["--kernel", "guards"]),
...
'''
            ...
            self.assertEqual(suite_calls, [("baseline", ["raytrace", "baseline"])])
